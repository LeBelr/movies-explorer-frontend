...
export default function Profile({ handleLogout, loggedIn, handleEditProfile, isOk }) {
  ...
  useEffect(() => {
    if (isValid.email && isValid.name && values.email !== currentUser.email && values.name !== currentUser.name) {
      setIsButtonActive(true);
    } else {
      setIsButtonActive(false);
    }
  }, [isValid.email, isValid.password, values.email, values.name]);
        ...
        <section className="profile" onSubmit={handleSubmit}>
          ...
          <form className="profile__form">
            ...
            <div className="profile__input-box">
              ...
              <input className="profile__input" id="email" type="email" name="email" placeholder="E-mail" required value={values.email} onChange={handleChange}></input>
